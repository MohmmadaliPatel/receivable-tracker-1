...
export async function GET(
  ...
  if (idx >= 0 && responses[idx]?.attachmentsJson) {
    attachmentMeta = parseAttachmentList(responses[idx].attachmentsJson).find((a) => a.id === attachmentId) ?? null;
  }
  if (!attachmentMeta && record.responseAttachmentsJson) {
    attachmentMeta = parseAttachmentList(record.responseAttachmentsJson).find((a) => a.id === attachmentId) ?? null;
  }

  if (attachmentMeta?.savedPath && fs.existsSync(attachmentMeta.savedPath)) {
    ...
    const isPdf =
      name.toLowerCase().endsWith('.pdf') || (attachmentMeta.contentType || '').toLowerCase().includes('pdf');
