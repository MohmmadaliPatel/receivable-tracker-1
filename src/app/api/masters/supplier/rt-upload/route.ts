...
/** POST /api/masters/supplier/rt-upload — RT India Sheet1 format; updates entity contacts + supplier master + TR + MSME listing rows */
export async function POST(request: NextRequest) {
  ...
  const tmp = path.join(os.tmpdir(), `rt-supplier-master-${Date.now()}.xlsx`);
  fs.writeFileSync(tmp, buf);
  try {
    const result = await syncRtIndiaContacts(tmp, 'supplier_only');
