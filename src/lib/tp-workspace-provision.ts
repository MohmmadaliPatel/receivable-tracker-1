...
async function trEligibleUserIds(): Promise<string[]> {
  const users = await prisma.user.findMany({
    where: { OR: [{ accessTradeReceivable: true }, { role: 'admin' }] },
    select: { id: true },
  });
  return users.map((u) => u.id);
}
...
function trRowCreatePayload(
  row: TradeReceivableConfirmation,
  targetUserId: string,
  emailThreadAnchorId: string | null
) {
  return {
    entityContactId: row.entityContactId,
    supplierMasterId: row.supplierMasterId,
    entityName: row.entityName,
    category: row.category,
    bankName: row.bankName,
    accountNumber: row.accountNumber,
...
/** Copy one user's TR listing workspace (anchors + invoice lines) to another user. */
export async function cloneTradeReceivablesFromUser(
  sourceUserId: string,
  targetUserId: string
): Promise<{ cloned: number }> {
  if (sourceUserId === targetUserId) return { cloned: 0 };

  const sourceRows = await prisma.tradeReceivableConfirmation.findMany({
    where: { userId: sourceUserId },
    orderBy: [{ createdAt: 'asc' }],
  });
  if (sourceRows.length === 0) return { cloned: 0 };

  await prisma.tradeReceivableConfirmation.deleteMany({ where: { userId: targetUserId } });
  ...
  for (const row of anchors) {
    const created = await prisma.tradeReceivableConfirmation.create({
      data: trRowCreatePayload(row, targetUserId, null),
    });
    idMap.set(row.id, created.id);
    cloned++;
  }
  ...
  for (const row of lines) {
    const anchorId = row.emailThreadAnchorId ? idMap.get(row.emailThreadAnchorId) ?? null : null;
    await prisma.tradeReceivableConfirmation.create({
      data: trRowCreatePayload(row, targetUserId, anchorId),
    });
    cloned++;
  }
...
export type ProvisionTradeReceivablesResult = {
  trUsers: number;
  trRowsCloned: number;
};
...
/**
 * After TR / supplier listing upload, clone the uploader's TR workspace to every
 * user with Trade Receivable access (or admin).
 */
export async function provisionTradeReceivableWorkspacesForAllEligibleUsers(
  sourceUserId: string
): Promise<ProvisionTradeReceivablesResult> {
  const result: ProvisionTradeReceivablesResult = {
    trUsers: 0,
    trRowsCloned: 0,
  };

  const sourceRowCount = await prisma.tradeReceivableConfirmation.count({
    where: { userId: sourceUserId },
  });
  if (sourceRowCount === 0) return result;

  for (const userId of await trEligibleUserIds()) {
    const { cloned } = await cloneTradeReceivablesFromUser(sourceUserId, userId);
    if (cloned > 0) {
      result.trUsers++;
      result.trRowsCloned += cloned;
    }
  }
